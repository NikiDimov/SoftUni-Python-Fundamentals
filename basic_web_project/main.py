...
def create_task_view(tk):
    ...
    Button(tk, text="Create new task", bg="green", fg="white",
...
def edit_task(task):
    ...
    Button(tk, text="Edit task", bg="yellow", fg="black", command=lambda: triger_edit(tk, selected, all_tasks,
                                                                                      [name.get(), date.get(),
                                                                                       description.get("1.0", END),
                                                                                       s.get(), chk_state.get()])).grid(
        row=5, column=0)
    Button(tk, text="Cancel", bg="black", fg="white", command=lambda: main_view(tk)).grid(row=5, column=1, padx=100,
                                                                                          pady=100)
...
def main_view(tk):
    clear_view(tk)
    Button(tk, text="List all tasks", bg="blue", fg="white", command=lambda: list_tasks_view(tk)).grid(row=1, column=0,
                                                                                                       padx=100,
                                                                                                       pady=100)
    Button(tk, text="Create new task", bg="green", fg="white", command=lambda: create_task_view(tk)).grid(row=1,
                                                                                                          column=1,
                                                                                                          padx=100,
                                                                                                          pady=100)
